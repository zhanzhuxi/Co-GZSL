...
class DATA_LOADER(object):
	...
	def read_matdataset(self, opt):
		...
		train_att_unique = self.attribute[train_id]
		self.att_seen = train_att_unique
		...
		seen_train_cla_num = self.ntrain_class - self.ntest_class
		seen_test_cla_num = self.ntest_class
		np.random.shuffle(self.seenclasses.numpy())
		seen_train_cla_idx = self.seenclasses[:seen_train_cla_num]
		seen_test_cla_idx = self.seenclasses[seen_train_cla_num:self.train_label.shape[0]]

		episode = []
		for c in seen_train_cla_idx.numpy():
			idx = np.argwhere(self.train_label == c).reshape(-1)
			episode.extend(idx)
		self.seen_train_feature = self.train_feature[episode]
		self.seen_train_label = self.train_label[episode]
		self.n_seen_train = self.seen_train_label.size(0)
		episode = []
		for c in seen_test_cla_idx.numpy():
			idx = np.argwhere(self.train_label == c).reshape(-1)
			episode.extend(idx)
		self.seen_test_feature = self.train_feature[episode]
		self.seen_test_label = self.train_label[episode]
		self.n_seen_test = self.seen_test_label.size(0)
	...
	def next_batch(self, batch_size):
		...
		seen_train_att_num = torch.from_numpy(np.unique(self.seen_train_label.numpy()))
		seen_test_att_num = torch.from_numpy(np.unique(self.seen_test_label.numpy()))
		batch_feature = self.train_feature[idx]
		batch_label = self.train_label[idx]
		batch_att = self.attribute[batch_label]
		return batch_feature, batch_label, batch_att

	def next_batch_(self, batch_size):
		idx = torch.randperm(self.seen_train_feature.size()[0])[0:batch_size]
		batch_feature = self.seen_train_feature[idx]
		batch_label = self.seen_train_label[idx]
		batch_att = self.attribute[batch_label]
		return batch_feature, batch_label, batch_att

	def next_batch_fine(self, batch_size):
		seen_test_feature = self.seen_test_feature
		seen_test_label = self.seen_test_label
		while seen_test_feature.size()[0] < batch_size:
			seen_test_feature = torch.cat((seen_test_feature, self.seen_test_feature), dim=0)
			seen_test_label = torch.cat((seen_test_label, self.seen_test_label), dim=0)
		idx = torch.randperm(seen_test_feature.size()[0])[0:batch_size]
		batch_feature = seen_test_feature[idx]
		batch_label = seen_test_label[idx]
		batch_att = self.attribute[batch_label]
		return batch_feature, batch_label, batch_att
	...
	def next_batch_class(self, M, N):
		episode = []
		selected_classes = np.random.choice(list(np.unique(self.train_label)), M, replace=False)
		for c in selected_classes:
			idx = np.argwhere(self.train_label == c).reshape(-1)
			l = np.random.choice(idx, N, replace=False)
			episode.extend(torch.from_numpy(l))

		batch_feature = self.train_feature[episode]
		batch_label = self.train_label[episode]
		batch_att = self.attribute[batch_label]
		return batch_feature, batch_label, batch_att
